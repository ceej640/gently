...
class EmbryoState:
    ...
    perception_beliefs: Optional[Dict] = None  # Serialized PerceptionSession from perception system
    perception_evidence_count: int = 0  # Number of observations accumulated
    ...
    def update_from_perception(self, session_dict: Dict, evidence_count: int = 0):
        """
        Update embryo state from perception session.
        ...
        ----------
        session_dict : dict
            Serialized PerceptionSession from perception system
        evidence_count : int
            Number of observations accumulated in session
        """
        self.perception_beliefs = session_dict
        self.perception_evidence_count = evidence_count

        # Get latest observation for current state
        observations = session_dict.get("observations", [])
        if observations:
            latest = observations[-1]
            self.latest_developmental_stage = latest.get("stage")

            # Update hatching status if hatched
            if latest.get("stage") == "hatched":
                self.hatching_status = {
                    "hatched": True,
                    "confidence": "HIGH",
                    "timepoint": session_dict.get("hatching_complete_at"),
                    "source": "perception",
                }
    ...
    def get_perception_summary(self) -> Optional[str]:
        ...
        session = self.perception_beliefs
        ...
        # Hatching state
        if session.get("hatching_complete_at"):
            lines.append(f"HATCHED at T{session['hatching_complete_at']}")
        elif session.get("hatching_started_at"):
            lines.append(f"Hatching since T{session['hatching_started_at']}")

        # Number of observations
        lines.append(f"{len(observations)} obs")
