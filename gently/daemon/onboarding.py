"""
Onboarding — the daemon's cold start conversation logic.

Bridges gap assessment → interaction tasks → context seeding.
This module generates the right questions and processes the answers
into structured context, based on what the daemon lacks.
"""
...
from ..context.gap_assessment import ContextGapReport, GapLayer
...
from .task import Task, TaskPriority, TaskType, make_task
...
def generate_onboarding_tasks(
    ...
    """
    Generate interaction tasks based on the gap assessment.

    Uses SURFACE (speak) tasks rather than ASK tasks so that questions
    appear as daemon messages in the CLI without blocking the scheduler.
    The researcher responds naturally through the copilot conversation,
    which has access to the daemon's context store.
    ...
    -------
    List[Task]
        Tasks to inject into the daemon's queue.
    """
    tasks = []

    if gap_report.needs_lab_onboarding:
        tasks.append(make_task(
            type=TaskType.SURFACE,
            target="researcher",
            reason="First launch — need to learn about the lab.",
            priority=TaskPriority.HIGH,
            params={"message": LAB_ONBOARDING_GREETING, "priority": "high"},
        ))
    ...
    if gap_report.needs_campaign:
        ...
        tasks.append(make_task(
            ...
            params={"message": prompt, "priority": "normal"},
        ))
    ...
    if tasks:
        logger.info(
            f"Generated {len(tasks)} onboarding tasks "
